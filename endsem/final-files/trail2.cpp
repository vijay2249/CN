...
void packet_injection_for_arp(){
    ...
    for(auto i: ARPTABLE){
        ...
        sprintf(tempdata,"%.2X-%.2X-%.2X-%.2X-%.2X-%.2X \t IP:%s\n", i.source_mac[0],i.source_mac[1],i.source_mac[2],i.source_mac[3],i.source_mac[4],i.source_mac[5], i.source_ip);
        printf("temp is %s\n",tempdata);
        data=(const u_char*)tempdata;
        // strcpy(data, i.source_ip);
        printf("%s\n", data);
        if(pcap_sendpacket(handle, data, 40) != 0){
