...
struct ARP_tab{
    ...
    char *source_ip;
}ARP_TABLE[total_cap_count];

// struct ARP_tab ARP[5];

void print_arp_table(int total_ips){
    printf("\n inside the print arp_table function\n");
    printf("total_ips value: %d", total_ips);
    for(int a=0;a<total_ips;a++){
        printf("IP%d: %.2X-%.2X-%.2X-%.2X-%.2X-%.2X\n", 
            total, ARP_TABLE[a].source_mac[0], ARP_TABLE[a].source_mac[1], ARP_TABLE[a].source_mac[2], ARP_TABLE[a].source_mac[3], ARP_TABLE[a].source_mac[4], ARP_TABLE[a].source_mac[5]);
    }
}
...
void add_to_arp_table(const u_char *buffer, int size){
    ...
    // int flag = 0;
    // if(temp_count == 0){
    //     struct ARP_tab arp_table;
    //     arp_table.source_mac = eth->h_source;
    //     struct iphdr *iph = (struct iphdr *)(buffer +sizeof(struct ethhdr));
    //     memset(&source, 0, sizeof(source));
    //     source.sin_addr.s_addr = iph->saddr;
    //     arp_table.source_ip = inet_ntoa(source.sin_addr);
    //     ARP_TABLE[temp_count] = arp_table;
    // }else{
    //     for(int a=0;a<temp_count;a++){
    //         if(eth->h_source == ARP_TABLE[a].source_mac){flag=1;break;}
    //     }
    //     if(!flag){
    //         struct ARP_tab arp_table;
    //         arp_table.source_mac = eth->h_source;
    ...
    // unsigned char *source_mac;
    // source_mac = eth->h_source;
    // if(temp_count == 0){
    //     ARP_TABLE[temp_count].source_mac = eth->h_source;
    //     struct iphdr *iph = (struct iphdr *)(buffer + sizeof(struct ethhdr));
    //     memset(&source, 0, sizeof(source));
    //     source.sin_addr.s_addr = iph->saddr;
    //     ARP_TABLE[temp_count].source_ip = inet_ntoa(source.sin_addr);
    // }else{
    //     int flag = 0;
    //     for(int a=0;a<temp_count;a++){
    //         if(eth->h_source == ARP_TABLE[i].source_mac) {flag = 1;break;}
    //     }
    //     if(!flag){
    //         ARP_TABLE[temp_count].source_mac = eth->h_source;
    //         struct iphdr *iph = (struct iphdr *)(buffer + sizeof(struct ethhdr));
    ...
    // printf("IP%d: %s\t\tMAC%d: %.2X-%.2X-%.2X-%.2X-%.2X-%.2X\n", 
    //         total, arp_table.source_ip, total, arp_table.source_mac[0], arp_table.source_mac[1], arp_table.source_mac[2], arp_table.source_mac[3], arp_table.source_mac[4], arp_table.source_mac[5]);
