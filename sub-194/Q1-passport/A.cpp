...
{ 
    ...
    strncpy(uaddr.sun_path, "/tmp/passing-fd.socket", sizeof(uaddr.sun_path)-1);
	...
	cout<<buf<<endl;
	 struct iphdr *ipp;
  	ipp=(struct iphdr*)buf;
  	 ...
  	 print_ipheader(ipp);//Printing Header Details
    ...
    //Again Recieve nsfd of client from 
     int * fd1=recv_fd(usfd);
    int nsfd=fd1[0];
    //Do all the Verification 
     char *hello = "Verification Done";
